...
std::vector<Ptr<BTensor>> NeuPIMSLogitSoftmax::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    // before making _nh, figure out the gqp
    // shaping q into k&v
    // head = _qs[0]->get_dims()[0];  // real _nh
    _nh = _ks[0]->get_dims()[0];
...
Tile NeuPIMSLogitSoftmax::initialize_instructions(int start, int end) {
    uint32_t banks_per_channel = _config.dram_banks_per_ch;  // 32
    ...
    for (int i = start; i < end + 1; i++) {
        ...
        for (int chunk = 0; chunk < _chunks; chunk++) {
            ...
            for (int ti = 0; ti < tiles_per_chunk; ti++) {
                ...
                uint32_t DRAM_row = key->_rows[ti * _chunks + chunk];  // got a problem when GQA?
