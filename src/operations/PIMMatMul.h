...
class PIMMatMul : public Operation {
   public:
    // PIMMatMul(std::string name, std::vector<uint32_t> weight_dim);
    PIMMatMul(std::string name, std::vector<Ptr<NPUTensor>> weights);
    PIMMatMul(std::string name);
