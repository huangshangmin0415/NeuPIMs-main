...
Swish::Swish(std::string name) : Operation(name) { _inputs.resize(1); }

// Gelu does not change shapes.
std::vector<Ptr<BTensor>> Swish::get_outputs(std::vector<Ptr<BTensor>> inputs) {
...
// table lookup
Tile Swish::initialize_instructions(uint32_t N) {
    auto tile = Tile{
...
uint32_t Swish::sram_size_needed() {
