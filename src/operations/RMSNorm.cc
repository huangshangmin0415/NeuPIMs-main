...
RMSNorm::RMSNorm(std::string name, std::vector<Ptr<NPUTensor>> weights) : Operation(name) {
...
// RMSNorm does not change shapes.
std::vector<Ptr<BTensor>> RMSNorm::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    // how to set the spdlog?
    spdlog::info("(RMSNrom)input dims : {} {}", _inputs[0]->get_dims(), _inputs[1]->get_dims());
    spdlog::info("(RMSNrom)output dim : {}", input_dims);
...
Tile RMSNorm::initialize_instructions(uint32_t N) {
    auto tile = Tile{
    ...
    // addr_type sram_gamma_base = ;
    addr_type sram_beta_base = SPAD_BASE;
    ...
    auto activation_tensor = std::static_pointer_cast<NPUTensor>(_inputs[0]);  // X
    auto gamma_tensor = std::static_pointer_cast<NPUTensor>(_inputs[1]);  // W
    auto output_tensor = std::static_pointer_cast<NPUTensor>(_outputs[0]);  // Y
    ...
    tile.instructions.push_back(Instruction{
        .opcode = Opcode::MOVIN,
        .dest_addr = sram_beta_base,  // sram_gamma_base
        // assume broadcasting bias is available inside the npu
    ...
    for (uint32_t n_inner_offset = 0; n_inner_offset < n_inner; ++n_inner_offset) {
        ...
        tile.instructions.push_back(Instruction{
            ...
            .src_addrs =
                std::vector<addr_type>{sram_activation_offset, sram_beta_base},
        });
        // -- save outputs --
...
uint32_t RMSNorm::sram_size_needed() {
    ...
    return k * (n * 3 + 1) * _config.precision;
}