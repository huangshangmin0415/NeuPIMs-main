...
std::vector<Ptr<BTensor>> NeuPIMSAttend::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    _nh = _vs[0]->get_dims()[0];  // why not logits?
    ...
    for (int i = 0; i < _batch_size; ++i) {
        ...
        std::vector<uint32_t> attend_output_dim{_nh, l, _dk};  // may smaller than we thought
        ...
        for (int hi = 0; hi < _nh; hi++) {
            ...
            for (int ci = 0; ci < chunks; ci++) {
                ...
                for (int ti = 0; ti < _tiles_per_chunk; ti++) {
                    ...
                    uint32_t DRAM_row = value->_rows[ti * chunks + ci];  // got a problem when GQA?
                    p_header_addr =
