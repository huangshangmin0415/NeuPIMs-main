...
#include "PIMPROJGEMV.h"

PIMPROJGEMV::PIMPROJGEMV(std::string name, std::vector<Ptr<NPUTensor>> weights) : Operation(name) {
...
std::vector<Ptr<BTensor>> PIMPROJGEMV::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    for (auto tensor : inputs) {
        if (i < _batch_size) {
            _qs.push_back(std::static_pointer_cast<NPUTensor>(tensor));  // data is now in _qs
        } else {
    ...
    _ks.push_back(std::static_pointer_cast<PIMTensor>(_inputs[1]));  // weight is now in _ks
    ...
    _dk = _qs[0]->get_dims()[1];  // actually E/tp
    ...
    for (int i = 0; i < _batch_size; ++i) {
        ...
        uint32_t seq_len = K->get_dims()[1];  // actually E
        uint32_t N = Q->get_dims()[0];

        // d_k of Q == d_k of K^T
        spdlog::info("data: {}, weight: {}", Q->get_dims(), K->get_dims());
        assert(Q->get_dims()[1] == K->get_dims()[0]);
        // std::vector<uint32_t> gemv_output_dim{_nh, 1, seq_len};
        std::vector<uint32_t> gemv_output_dim{N, seq_len};
    ...
    spdlog::info("safely reach end of PIMPROJGEMV");

    return _outputs;
}

void PIMPROJGEMV::initialize_tiles() { _tiles.push_back(initialize_instructions()); }

Tile PIMPROJGEMV::initialize_instructions() {
    ...
    for (int i = 0; i < _batch_size; i++) {
        ...
        spdlog::info("pim_gemv MOVOUT begin, _nh: {}", _nh);  // oh, no!
...
void PIMPROJGEMV::calculate_loops() {
...
uint32_t PIMPROJGEMV::sram_size_needed() {
