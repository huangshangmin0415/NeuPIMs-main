...
// PIMMatMul::PIMMatMul(std::string name, std::vector<uint32_t> weight_dim) : Operation(name) {
...
/**
 * Initialize PIMMatMul
 *  - if it has bias, weights has two pointer -> resize input to 2
 *  - else if one -> resize input to 2 but not the case in GPT2
 *  - Case where PIMMatMul doesn't have weight, it is initialized only with name. (Below Constructor)
 */
PIMMatMul::PIMMatMul(std::string name, std::vector<Ptr<NPUTensor>> weights) : Operation(name) {
...
PIMMatMul::PIMMatMul(std::string name) : Operation(name) { _inputs.resize(2); }

/**
 * function executing PIMMatMul
 *  inputs:
 *      If weight exist, only has a single tensor. else, has two tensors
 *      Case where PIMMatMul getting a single tensor input is calculating QKV, proj, MLP layer
 ...
 */
std::vector<Ptr<BTensor>> PIMMatMul::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        _inputs[i] = inputs[i];
        spdlog::info("PIMMatMul input idx: {} / input sz: {}", i, inputs[i]->get_dims());
    }
    ...
    *(output_dims.rbegin() + 1) =
        *(input0_dims.rbegin() + 1);                // Set (M, x) in PIMMatMul (M, K) x (K, N).
    *output_dims.rbegin() = *input1_dims.rbegin();  // Set (x, N) in PIMMatMul (M, K) x (K, N)
    spdlog::info("PIMMatMul output sz: {}", output_dims);
...
void PIMMatMul::initialize_tiles() {
    // Here, B does not refer to batch_size,
    // but rather to the outer dimensions of the tensor in PIMMatMul,
...
Tile PIMMatMul::initialize_instructions(uint32_t B, uint32_t M, uint32_t K, uint32_t N,
                                     bool should_store) {
    auto tile = Tile{
    ...
    // spdlog::info("start tile initiation");
    ...
    if (_inputs.size() == 3 && K == 0) {
        ...
        tile.instructions.push_back(Instruction{
            ...
            .size = 0,
            .src_addrs = std::vector<addr_type>{bias_tensor->get_addr({n_outer_offset})},
        ...
        for (uint32_t n_inner_offset = 0; n_inner_offset < n_inner; n_inner_offset += loop_size) {
            ...
            // PIM_GWWRITE, or PIM_HEADER
            tile.instructions.push_back(Instruction{
                .opcode = Opcode::PIM_HEADER,
                .dest_addr = sram_accumulation_base,
                .size = 0,
                .src_addrs = std::vector<addr_type>{bias_tensor->get_addr({n_outer_offset + n_inner_offset})},
                .operand_id = _INPUT_OPERAND + 2,
            });
            ...
            for (uint32_t n_loop = 0; n_loop < loop_size; ++n_loop) {
                ...
                if (bias_addr != GARBAGE_ADDR) {
                    ...
                    tile.instructions.push_back(Instruction{
                        ...
                        .dest_addr = sram_accumulation_base + (n_inner_offset + n_loop) * _config.precision,
                        .size = (uint32_t)_config.precision,
                        .src_addrs = std::vector<addr_type>{bias_addr},
                        .operand_id = _INPUT_OPERAND + 2,
                    });
                }
            }
    ...
    tile.instructions.push_back(Instruction{
        ...
        .dest_addr = sram_accumulation_base,
        .size = 0,
        .src_addrs = std::vector<addr_type>{activation_tensor->get_addr({m_outer_offset, k_outer_offset})},
        .operand_id = _INPUT_OPERAND,
    });

    tile.instructions.push_back(Instruction{
        .opcode = Opcode::PIM_GWRITE,
        .dest_addr = sram_weight_base,
        .size = 0,
        .src_addrs = std::vector<addr_type>{weight_tensor->get_addr({k_outer_offset, n_outer_offset})},
            ...
            for (uint32_t m_inner_offset = 0; m_inner_offset < m_inner; m_inner_offset += loop_size) {
                ...
                if (n_inner_offset == 0) {
                    ...
                    tile.instructions.push_back(Instruction{
                        ...
                        .size = 0,
                        .src_addrs = std::vector<addr_type>{activation_tensor->get_addr({m_outer_offset + m_inner_offset, k_outer_offset + k_inner_offset})},
                        .operand_id = _INPUT_OPERAND,
                    });
                        ...
                        for (int k_loop = 0; k_loop < loop_size; k_loop++) {
                            ...
                            if (activation_addr != GARBAGE_ADDR) {
                                ...
                                if (m_loop == 0)
                                    tile.instructions.push_back(Instruction{
                                        .opcode = Opcode::PIM_COMPS_READRES,
                                        .dest_addr = sram_activation_base + ((m_inner_offset + m_loop) * k_inner + (k_inner_offset + k_loop)) * _config.precision,
                                        .size = (uint32_t)loop_size * _config.precision,
                                        .src_addrs = std::vector<addr_type>{activation_addr},
                                        .operand_id = _INPUT_OPERAND,
                                    });
                            }
                        }
                ...
                // spdlog::info("activation finished");
                ...
                if (m_inner_offset == 0) {
                    ...
                    tile.instructions.push_back(Instruction{
                        ...
                        .size = 0,
                        .src_addrs = std::vector<addr_type>{weight_tensor->get_addr({k_outer_offset + k_inner_offset, n_outer_offset + n_inner_offset})},
                        .operand_id = _INPUT_OPERAND + 1,
                    });
                        ...
                        for (int n_loop = 0; n_loop < loop_size; n_loop++) {
                            ...
                            if (weight_addr != GARBAGE_ADDR) {
                                ...
                                if (n_loop == 0)
                                    tile.instructions.push_back(Instruction{
                                        .opcode = Opcode::PIM_COMPS_READRES,
                                        .dest_addr = sram_weight_base + ((k_inner_offset + k_loop) * n_inner + (n_inner_offset + n_loop)) * _config.precision,
                                        .size = (uint32_t)loop_size * _config.precision,
                                        .src_addrs = std::vector<addr_type>{weight_addr},
                                        .operand_id = _INPUT_OPERAND + 1,
                                    });
                            }
                        }
...
// _outer_loop indicates how many L2 tiles the PIMMatMul is divided into across each axis M, N, K.
void PIMMatMul::calculate_loops() {
    ...
    // Calculate the number of iterations for PIMMatMul by multiplying all dimensions except the last
    // two.
    _prod_batches = 1;
    ...
    spdlog::info("PIMMatMul inner loop: {}, outer loop: {}", _inner_loop, _outer_loop);
...
uint32_t PIMMatMul::sram_size_needed() {
