...
void Scheduler::allocate_requests() {
    ...
    for (auto it = _request_queue.begin(); it != _request_queue.end(); it++) {
        ...
        if (!request->is_initiated) {
            ...
            std::vector<uint32_t> dim_key{_nh / _qgp, _dk, seq_len};
            std::vector<uint32_t> dim_value{_nh / _qgp, seq_len, _dk};
...
void Scheduler::make_program() {
    ...
    _model_program1 =
        std::make_unique<StageProgram>(_model, sub_batch_on_sa, StagePlatform::SA, _stage);  // StagePlatform::SA
    _model_program2 =
        std::make_unique<StageProgram>(_model, sub_batch_on_pim, StagePlatform::PIM, _stage);  // StagePlatform::PIM
...
Tile& Scheduler::top_tile1(uint32_t core_id) {
    ...
    if (_executable_tile_queue1.empty()) {
        ...
        if (tile.status == Tile::Status::BAR) {
            ...
            tile.stage_platform = StagePlatform::SA;  // StagePlatform::SA
            return tile;
        }
    }
}
