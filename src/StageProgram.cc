...
void StageProgram::init_PIM_program() {
    ...
    // uint32_t num_heads = (Config::global_config.model_n_head / Config::global_config.gqp) / Config::global_config.n_tp;
...
std::vector<Ptr<BTensor>> StageProgram::projection_block(std::vector<Ptr<BTensor>> inputs) {
    ...
    auto projection = add_op(std::make_shared<MatMul>(  // MatMul, but not totally replaced
...
std::vector<Ptr<BTensor>> StageProgram::ffn1_block(std::vector<Ptr<BTensor>> inputs) {
    ...
    auto rn = add_op(std::make_shared<RMSNorm>(
        name_gen(prefix, OperationType::RMSNorm),
        _model->get_params(layer, BlockType::FeedForward, OperationType::RMSNorm)));
    inputs = get_outputs(rn, inputs);

    auto fc1 = add_op(std::make_shared<MatMul>(  //MatMul
    ...
    auto swish = add_op(std::make_shared<Swish>(name_gen(prefix, OperationType::Swish)));
    inputs = get_outputs(swish, inputs);

    auto fc2 = add_op(std::make_shared<MatMul>(  //MatMul
...
std::vector<Ptr<BTensor>> StageProgram::qkv_gen_block(std::vector<Ptr<BTensor>> inputs) {
    ...
    auto rn1 = add_op(std::make_shared<RMSNorm>(
        name_gen(prefix, OperationType::RMSNorm),
        _model->get_params(layer, BlockType::Attention, OperationType::RMSNorm)));
    inputs = get_outputs(rn1, inputs);

    // (N,E) x (E,3E)
    auto qkv_gen = add_op(std::make_shared<MatMul>( // MatMul
