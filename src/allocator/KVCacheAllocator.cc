...
void KVCacheAlloc::init_npu_layout(addr_type base_addr) {
    ...
    uint32_t gqp = Config::global_config.gqp;
    ...
    _kv_cache_size = max_active_reqs * max_seq_len * (h / gqp) * d_k * precision;
    ...
    uint64_t num_kv_cache_entries = max_active_reqs * max_seq_len * (h / gqp) / _kv_cache_entry_size;
