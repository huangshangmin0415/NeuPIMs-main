...
void NeuPIMSystolicWS::systolic_cycle() {
    ...
    if (!_compute_pipeline.empty() && _compute_pipeline.front().finish_cycle <= _core_cycle) {
        ...
        if (inst.dest_addr >= ACCUM_SPAD_BASE) {
            ...
            //              inst.accum_spad_id);
            buffer->fill(inst.dest_addr, inst.accum_spad_id);  // should be pim_acc_spad
            // spdlog::info("compute pipeline spad filled");
        } else {
            assert(0);
        }
...
// void NeuPIMSystolicWS::pim_ld_queue_cycle() {  // to be abandoned
//     /* LD instruction queue */
//     // todo: ld_queue.cycle();
//     std::vector<uint32_t> ch_req_dist(_config.dram_channels, 0);
//     bool filled = false;
...
//             auto accesses = MemoryAccess::from_instruction(
//                 front, generate_mem_access_id(), _config.dram_req_size, MemoryAccessType::READ,
//                 true, _id, _core_cycle, buffer_id, StagePlatform::PIM);  // st is ok; so ld is ok, too
...
//             // spdlog::info("pim push_memory_request");
//             _ld_inst_queue_for_pim.pop();
...
cycle_type NeuPIMSystolicWS::get_vector_compute_cycles(Instruction &inst) {
    ...
    switch (inst.opcode) {
        case Opcode::LAYERNORM:
            ...
            // spdlog::info("standard calculation cycles: {}", add_tree + scalar_ops + vector_ops);
        ...
        case Opcode::SWISH:
            return vec_op_iter * _config.swish_latency;
        case Opcode::DUMMY:
            return 1;
    }
...
void NeuPIMSystolicWS::issue_ex_inst(Instruction inst) {
    ...
    if (inst.opcode == Opcode::GEMM || inst.opcode == Opcode::GEMM_PRELOAD) {
        ...
        _stat_systolic_inst_issue_count++;
               ...
               inst.opcode == Opcode::LAYERNORM || inst.opcode == Opcode::RMSNORM ||
               inst.opcode == Opcode::SOFTMAX || inst.opcode == Opcode::ADD ||
               inst.opcode == Opcode::GELU || inst.opcode == Opcode::SWISH ||
...
void NeuPIMSystolicWS::pim_issue_ex_inst(Instruction inst) {
    ...
    if (inst.opcode == Opcode::GEMM || inst.opcode == Opcode::GEMM_PRELOAD) {
        // xxx: not yet for pim.
        assert(0);  // may calculate cycles differently from sa
        auto parent_tile = inst.parent_tile.lock();
        if (parent_tile == nullptr) {
            assert(0);
        }
