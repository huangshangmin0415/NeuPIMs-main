...
std::string PIMPROJGEMV = "PIMPROJGEMV";
std::string PIMMatMul = "PIMMatMul";
}  // namespace OperationType
...
void Model::init_params() {
    for (int i = 0; i < _config.model_n_layer; ++i) {
        ...
        //               {_config.model_n_embd});
        create_weight(name_gen(attn, OperationType::RMSNorm, ParameterType::Weight),
                      {_config.model_n_embd});
        create_weight(name_gen(attn, OperationType::RMSNorm, ParameterType::Bias),
                      {_config.model_n_embd});
        ...
        //               {_config.model_n_embd});
        create_weight(name_gen(ffn, OperationType::RMSNorm, ParameterType::Weight),
                      {_config.model_n_embd});
        create_weight(name_gen(ffn, OperationType::RMSNorm, ParameterType::Bias),
                      {_config.model_n_embd});
...
std::shared_ptr<Tensor> Model::load_cache(uint32_t layer, std::string type) {
    ...
    if (type == "key") {
        shape.assign(
            {_config.model_n_head / _config.gqp, _config.model_n_embd / _config.model_n_head, _num_token});
    } else if (type == "value") {
        shape.assign(
            {_config.model_n_head / _config.gqp, _num_token, _config.model_n_embd / _config.model_n_head});
    }
