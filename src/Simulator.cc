...
void Simulator::cycle() {
    ...
    while (running()) {
        ...
        if (_cycle_mask & CORE_MASK) {
            ...
            for (int core_id = 0; core_id < _n_cores; core_id++) {
                ...
                if (!_scheduler->empty1()) {
                    ...
                    if ((tile.status != Tile::Status::EMPTY) && _cores[core_id]->can_issue(tile)) {  // can_issue(tile)
                        if (tile.status == Tile::Status::INITIALIZED) {
                            assert(tile.stage_platform == StagePlatform::SA);
                            _cores[core_id]->issue(tile);  // issue
        ...
        if (_cycle_mask & ICNT_MASK) {
            // spdlog::info("simulator in the interconnct cycle");
            for (int core_id = 0; core_id < _n_cores; core_id++) {
                for (uint32_t channel_index = 0; channel_index < _n_memories; ++channel_index) {
                    auto core_ind = core_id * _n_cores + channel_index;  // always assume core_ind == channel_index
