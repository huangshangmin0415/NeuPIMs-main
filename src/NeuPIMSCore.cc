...
bool NeuPIMSCore::can_issue(Tile &next_tile) {
    ...
    if (!next_tile.accum) {  // for most tiles created by the big operations
...
void NeuPIMSCore::issue_pim(Tile &in_tile) {
    ...
    for (auto &inst : tile->instructions) {
        ...
        // spdlog::info("break point 2");

        if (inst.opcode == Opcode::PIM_HEADER) {
            _ld_inst_queue_for_pim.push(inst);
            // spdlog::info("break point 3.1");
        } else if (inst.opcode == Opcode::MOVIN || inst.opcode == Opcode::PIM_GWRITE ||
                   inst.opcode == Opcode::PIM_COMP || inst.opcode == Opcode::PIM_READRES ||
                   inst.opcode == Opcode::PIM_COMPS_READRES) {
            if (!buffer->check_allocated(inst.dest_addr, buffer_id) &&
                buffer->check_remain(inst.size, buffer_id)) {
                tile->remaining_loads++;
                // spdlog::info("remaining_loads increased");
                _ld_inst_queue_for_pim.push(inst);
            } else {
            ...
            // spdlog::info("break point 3.2");
        } else {
            /* Ex inst queue */
            tile->remaining_accum_io++;
            tile->remaining_computes++;
            _ex_inst_queue_for_pim.push(inst);
            // spdlog::info("break point 3.3");
        }
    }
    // spdlog::info("tile pushed to core._tiles {}", tile.repr());
...
bool NeuPIMSCore::running() {
    ...
    // spdlog::info("tiles size: {}", _tiles.size());
    // spdlog::info("compute pipeline empty: {}", _compute_pipeline.empty());
    // spdlog::info("running except for temp: {}", temp);
    // spdlog::info("waiting write reqs empty: {}", _waiting_write_reqs == 0);
...
void NeuPIMSCore::push_memory_response(MemoryAccess *response) {
    ...
    // spdlog::info("memory response arrived");

    Sram *acc_spad = &_acc_spad;
    Sram *spad = &_spad;
    // Sram *acc_spad = &_pim_acc_spad;
    // Sram *spad = &_pim_spad;
    uint32_t buf_id;
    ...
    // spdlog::info("memory access type: {}", (int)response->req_type);
    ...
    if (auto tile = response->parent_tile.lock()) {
        if (is_write) {
            ...
            tile->remaining_loads--;
            // spdlog::info("remaining_loads declined");
        }
    } else {
        assert(0);
    }
    if (is_write) {
        ...
        // spdlog::info("comps_readres finished fill higher addr");
    } else {
        ...
        // spdlog::info("comps_readres finished fill lower addr");
    }
    delete response;
}
...
void NeuPIMSCore::print_stats() {
    spdlog::info(
        "NeuPIMSCore [{}] : MatMul cycle {} RMSNorm cycle {} Softmax cycle {} "
        "Add cycle {}  Swish cycle {}",
        _id, _stat_matmul_cycle, _stat_rmsnorm_cycle, _stat_softmax_cycle, _stat_add_cycle,
        _stat_swish_cycle);
    spdlog::info(
        "NeuPIMSCore [{}] : MatMul stall cycle {} RMSNorm stall cycle {} "
        "Softmax stall cycle {} Add stall cycle {} Swish stall cycle {}",
        _id, _compute_memory_stall_cycle, _rmsnorm_stall_cycle, _softmax_stall_cycle,
        _add_stall_cycle, _swish_stall_cycle);
